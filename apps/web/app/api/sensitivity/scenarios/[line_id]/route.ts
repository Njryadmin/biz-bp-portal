// apps/web/app/api/sensitivity/scenarios/[line_id]/route.ts
// BFF proxy: forwards GET /api/sensitivity/scenarios/{line_id} to the Python API.
...
export async function GET(
  ...
  try {
    const res = await fetch(`${base}/api/sensitivity/scenarios/${lineId}`, {
      cache: "no-store",
    });
    if (!res.ok) {

...
//
// BFF proxy: PATCH /api/admin/tenants/{id} → upstream.
//
// M3 (2026-09-04) — super-admin only. The router on the Python
// side accepts only PATCH (slug is immutable, so PUT / DELETE don't
// make sense for now). Pattern copied from
...
async function proxy(request: Request, ctx: Ctx): Promise<Response> {
  const tenantId = encodeURIComponent(ctx.params.id);
